...
`build/libs/OpenZen-acmuhan-1.0.0.jar` 可以放进 `mods/`，但 OpenZen 不是纯普通 Forge Mod：核心功能必须在 Minecraft 类加载前安装 patch。当前版本会按以下顺序尝试：

1. **ModLauncher LaunchPlugin**：普通 `mods/` 加载时优先走这个路径，尽量在类加载阶段直接改字节码，不依赖 `-javaagent`。
2. **DLL 注入器**：`OpenZenLoader.exe` 会在运行中的 JVM 里 attach agent。
3. **Gradle 开发路径**：`runClient0` 会自动追加 `-javaagent:<jar>`。
4. **JDK Attach fallback**：如果前面没生效，会尝试用 JDK Attach API 自动 attach 当前 jar。

如果这些路径都失败，游戏内会弹出 **“OpenZen 补丁未生效”** 提示。出现该提示时，说明当前只是普通 Forge Mod 被加载，核心 patch 没装上；请优先使用 Actions 产物里的 `OpenZenLoader.exe`，或者在启动器 **Java 虚拟机参数** 手动加入：

```text
-javaagent:<OpenZen-acmuhan-1.0.0.jar 的绝对路径> -Djdk.attach.allowAttachSelf=true
```

Forge 版本方面，本项目当前面向 Minecraft `1.20.1` / Forge `47.x`，本地构建使用 `47.4.20`。只要是 Forge 47.x，问题通常不是 Forge 小版本，而是 patch 是否真的安装成功。
