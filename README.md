...
`build/libs/OpenZen-acmuhan-1.0.0.jar` 不能只丢进 `mods/` 后直接使用。OpenZen 的核心 patch 依赖 `java.lang.instrument.Instrumentation`：

- DLL 注入路径通过 `OpenZenLoader.exe` 在运行中的 JVM 里 attach agent。
- Gradle 开发路径通过 `runClient0` 自动追加 `-javaagent:<jar>`。
- 普通 Forge `mods/` 加载只会执行 `@Mod`，不会触发 `Premain-Class`，因此日志会出现 `PatchAgent not attached`，patch 不会安装，右 Shift / ClickGUI 等功能也就不可用。

如果要走标准 Forge 启动器的 mod jar 路径，请同时：

1. 把 `OpenZen-acmuhan-1.0.0.jar` 放进对应实例的 `mods/`。
2. 在启动器 JVM 参数里额外加入：
