...
`build/libs/OpenZen-acmuhan-1.0.0.jar` 可以直接放进 `mods/`。OpenZen 的核心 patch 依赖 `java.lang.instrument.Instrumentation`，启动时会按以下顺序处理：

- 如果通过 DLL 注入器启动，`OpenZenLoader.exe` 会在运行中的 JVM 里 attach agent。
- 如果通过 Gradle 开发路径启动，`runClient0` 会自动追加 `-javaagent:<jar>`。
- 如果只是普通 Forge `mods/` 加载，OpenZen 会尝试使用 JDK Attach API 自动把当前 jar attach 到当前 JVM。

自动 attach 需要启动器使用的是完整 JDK/JRE 17 且运行时允许本进程 attach。大多数 Java 17 启动环境可用；如果启动器精简了 `jdk.attach` 模块或运行时禁止 self-attach，则日志会出现 `PatchAgent self-attach failed` / `PatchAgent not attached`，这时再手动添加 JVM 参数：

```text
-javaagent:<OpenZen-acmuhan-1.0.0.jar 的绝对路径> -Djdk.attach.allowAttachSelf=true
```

Forge 版本方面，本项目当前面向 Minecraft `1.20.1` / Forge `47.x`，本地构建使用 `47.4.20`。只要是 Forge 47.x，问题通常不是 Forge 小版本，而是是否以 `-javaagent` / 注入方式提供了 Instrumentation。
